...
//
#include "JetCorrectorParameters_tm.h"
...
//------------------------------------------------------------------------
JetCorrectorParameters_tm::Definitions::Definitions(const std::vector<std::string>& fBinVar, const std::vector<std::string>& fParVar, const std::string& fFormula, bool fIsResponse)
{
...
//------------------------------------------------------------------------
//--- JetCorrectorParameters_tm::Definitions constructor --------------------
//--- reads the member variables from a string ---------------------------
//------------------------------------------------------------------------
JetCorrectorParameters_tm::Definitions::Definitions(const std::string& fLine)
{
      ...
      if (tokens.size() < 6) 
        {
          ...
          handleError("JetCorrectorParameters_tm::Definitions",sserr.str());
        }
...
//------------------------------------------------------------------------
//--- JetCorrectorParameters_tm::Record constructor -------------------------
//--- reads the member variables from a string ---------------------------
//------------------------------------------------------------------------
JetCorrectorParameters_tm::Record::Record(const std::string& fLine,unsigned fNvar) : mMin(0),mMax(0)
{
  mNvar = fNvar;
  ...
  if (!tokens.empty())
    { 
	  ...
	  sserr<<"(line "<<fLine<<"): "<<"three tokens expected, "<<tokens.size()<<" provided.";
          handleError("JetCorrectorParameters_tm::Record",sserr.str());
        }
...
//------------------------------------------------------------------------
//--- JetCorrectorParameters_tm constructor ---------------------------------
//--- reads the member variables from a string ---------------------------
//------------------------------------------------------------------------
JetCorrectorParameters_tm::JetCorrectorParameters_tm(const std::string& fFile, const std::string& fSection) 
{
  ...
  if (currentDefinitions=="")
    handleError("JetCorrectorParameters_tm","No definitions found!!!");
  ...
  if (mRecords.empty() && currentSection != "") 
    {
      ...
      handleError("JetCorrectorParameters_tm",sserr.str()); 
    }
...
//------------------------------------------------------------------------
int JetCorrectorParameters_tm::binIndex(const std::vector<float>& fX) const 
{
  int result = -1;
...
//------------------------------------------------------------------------
int JetCorrectorParameters_tm::neighbourBin(unsigned fIndex, unsigned fVar, bool fNext) const 
{
  int result = -1;
  unsigned N = mDefinitions.nBinVar();
  if (fVar >= N) 
...
//------------------------------------------------------------------------
unsigned JetCorrectorParameters_tm::size(unsigned fVar) const
{
...
//------------------------------------------------------------------------
std::vector<float> JetCorrectorParameters_tm::binCenters(unsigned fVar) const 
{
...
//------------------------------------------------------------------------
void JetCorrectorParameters_tm::printScreen() const
{
  std::cout<<"--------------------------------------------"<<std::endl;
...
//------------------------------------------------------------------------
void JetCorrectorParameters_tm::printFile(const std::string& fFileName) const
{
...
const char * 
JetCorrectorParameters_tmCollection::labelsArray_[JetCorrectorParameters_tmCollection::N_LEVELS] = 
  {
    "L1Offset",
    "L2Relative",
    "L3Absolute",
...
const char *
JetCorrectorParameters_tmCollection::l5FlavorArray_[JetCorrectorParameters_tmCollection::N_L5_SPECIES] = 
  {
    "L5Flavor_bJ",
    "L5Flavor_cJ",
    "L5Flavor_qJ",
...
const char *
JetCorrectorParameters_tmCollection::l7PartonArray_[JetCorrectorParameters_tmCollection::N_L7_SPECIES] = 
  {
    "L7Parton_gJ",
    "L7Parton_qJ",
    "L7Parton_cJ",
...
//int sizeBuffer =  sizeof(labelsArray_)/sizeof(*labelsArray_);
std::vector<std::string>
JetCorrectorParameters_tmCollection::labels_(labelsArray_, labelsArray_ + sizeof(labelsArray_)/sizeof(*labelsArray_));

std::vector<std::string>
JetCorrectorParameters_tmCollection::l5Flavors_(l5FlavorArray_, l5FlavorArray_ + sizeof(l5FlavorArray_)/sizeof(*l5FlavorArray_) );

std::vector<std::string>
JetCorrectorParameters_tmCollection::l7Partons_(l7PartonArray_, 
					     l7PartonArray_ + sizeof(l7PartonArray_)/sizeof(*l7PartonArray_) );

void JetCorrectorParameters_tmCollection::getSections( std::string inputFile,
...
void JetCorrectorParameters_tmCollection::push_back( key_type i, value_type const & j, label_type const & flav) { 
  if ( isL5(i) )
...
JetCorrectorParameters_tm const & JetCorrectorParameters_tmCollection::operator[]( key_type k ) const {
...
void JetCorrectorParameters_tmCollection::validKeys(std::vector<key_type> & keys ) const {
  keys.clear();
...
JetCorrectorParameters_tmCollection::key_type
JetCorrectorParameters_tmCollection::getL5Bin( std::string const & flav ){
...
JetCorrectorParameters_tmCollection::key_type
JetCorrectorParameters_tmCollection::getL7Bin( std::string const & flav ){
  std::vector<std::string>::const_iterator found = 
...
bool JetCorrectorParameters_tmCollection::isL5( key_type k ) {
  return k == L5Flavor ||
    ( k / 10 > 0 && k / 1000 == 0 );
}
// Check if this is an L7 hashed value
bool JetCorrectorParameters_tmCollection::isL7( key_type k ) {
  return k == L7Parton ||
    ( k / 1000 > 0 );
}
...
JetCorrectorParameters_tmCollection::key_type 
JetCorrectorParameters_tmCollection::findKey( std::string const & label ) const {
  ...
  throw std::runtime_error(" cannot find label ");
...
// TYPELOOKUP_DATA_REG(JetCorrectorParameters_tm);
// TYPELOOKUP_DATA_REG(JetCorrectorParameters_tmCollection);
